...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "datpaq",
		Short: "Manage datpaq resources via the datpaq API",
		Long: `Manage datpaq resources via the datpaq API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'datpaq doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("datpaq {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'datpaq profile list')")
	...
	rootCmd.AddCommand(newTextLanguageCmd(flags))
	rootCmd.AddCommand(newUnitConversionCmd(flags))
	...
	rootCmd.AddCommand(newCompanyEnrichmentPromotedCmd(flags))
	rootCmd.AddCommand(newConvertTimePromotedCmd(flags))
	rootCmd.AddCommand(newCurrentTimePromotedCmd(flags))
	rootCmd.AddCommand(newDefinePromotedCmd(flags))
	rootCmd.AddCommand(newDnsPromotedCmd(flags))
	rootCmd.AddCommand(newGeneratePromotedCmd(flags))
	rootCmd.AddCommand(newGenerateBatchPromotedCmd(flags))
	rootCmd.AddCommand(newMacAddressPromotedCmd(flags))
