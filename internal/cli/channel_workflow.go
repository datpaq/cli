...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  datpaq workflow archive

  # Full re-archive (ignore previous sync state)
  datpaq workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"country-codes", "country-codes-countries", "ev-charger", "helicopter", "image-processing", "ip-intelligence", "ip-intelligence-batch-info", "mac-address", "precious-metals", "precious-metals-prices", "public-holidays", "schemas", "spell-check", "us-states", "text-language", "unit-conversion", "vin-lookup"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  datpaq workflow status

  # Show status as JSON
  datpaq workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("datpaq")
			}
