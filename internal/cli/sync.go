...
Resource scoping:
  ...
  datpaq sync --resources channels,messages
  ...
  datpaq sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  datpaq sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/datpaq/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "country-codes":
		return db.UpsertCountryCodes(data)
	case "domain-lookup":
		return db.UpsertDomainLookup(data)
	case "email-validation":
		return db.UpsertEmailValidation(data)
	case "ev-charger":
		return db.UpsertEvCharger(data)
	case "exchange-rates-and-currency":
		return db.UpsertExchangeRatesAndCurrency(data)
	case "helicopter":
		return db.UpsertHelicopter(data)
	case "image-processing":
		return db.UpsertImageProcessing(data)
	case "ip-geolocation":
		return db.UpsertIpGeolocation(data)
	case "ip-intelligence":
		return db.UpsertIpIntelligence(data)
	case "mx-lookup":
		return db.UpsertMxLookup(data)
	case "precious-metals":
		return db.UpsertPreciousMetals(data)
	case "profanity":
		return db.UpsertProfanity(data)
	case "public-holidays":
		return db.UpsertPublicHolidays(data)
	case "sample-data":
	...
	case "unit-conversion":
		return db.UpsertUnitConversion(data)
	case "user-avatar":
		return db.UpsertUserAvatar(data)
	case "validate-ip":
		return db.UpsertValidateIp(data)
	case "vin-lookup":
		return db.UpsertVinLookup(data)
	case "working-days":
		return db.UpsertWorkingDays(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"country-codes":              "/country-codes/export",
		"country-codes-countries":    "/country-codes/countries",
		"ev-charger":                 "/ev-charger/networks",
		"helicopter":                 "/helicopter/manufacturers",
		"image-processing":           "/image-processing",
		"ip-intelligence":            "/ip-intelligence",
		"ip-intelligence-batch-info": "/ip-intelligence/batch/info",
