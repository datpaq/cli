...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DATPAQ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DATPAQ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "datpaq/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.datpaq/feedback.jsonl.
When ` + "`DATPAQ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DATPAQ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  datpaq feedback list
  datpaq feedback list --limit 5
  datpaq feedback list --json`,
