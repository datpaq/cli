...
In local mode: searches locally synced data only.`,
  ...
  datpaq search "error timeout"

  # Force local search only
  datpaq search "payment failed" --data-source local

  # Search a specific resource type locally
  datpaq search "critical" --type transactions --data-source local

  # JSON output for piping
  datpaq search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /ev-charger/stations/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/ev-charger/stations/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'datpaq sync' first to populate the local database.", err)
			}
			defer db.Close()
