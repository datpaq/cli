...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  datpaq doctor
  datpaq doctor --json
  datpaq doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export DATPAQ_API_KEY=<your-key>"
					report["auth_docs_url"] = "https://datpaq.com"
				} else {
					authConfigured = true
			...
			// Check both the documented env var and its legacy alias.
			if v := os.Getenv("DATPAQ_API_KEY"); v != "" {
				authEnvSet = append(authEnvSet, "DATPAQ_API_KEY")
			} else if v := os.Getenv("DATPAQ_API_KEY_HEADER"); v != "" {
				authEnvSet = append(authEnvSet, "DATPAQ_API_KEY_HEADER")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "DATPAQ_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "datpaq", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'datpaq sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'datpaq sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'datpaq sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'datpaq sync' to refresh."
	}
	return report
}
