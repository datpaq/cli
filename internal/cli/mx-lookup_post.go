	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("resolve-ips") {
					body["resolve_ips"] = bodyResolveIps
				}
				if cmd.Flags().Changed("test-smtp") {
					body["test_smtp"] = bodyTestSmtp
				}
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "mx-lookup", partialFailure.Message))
						}
						return nil
					}
				} else {
